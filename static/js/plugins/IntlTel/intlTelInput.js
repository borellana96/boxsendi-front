...
class Iti {
  ...
  _appendListItems(countries, className) {
    ...
    for (let i = 0; i < countries.length; i++) {
      const c = countries[i]
      // open the list item
      tmp += `<li class='country ${className}' id='iti-item-${
        c.iso2
      }' role='option' data-dial-code='${c.dialCode}' data-country-code='${
        c.iso2
      }'>`
      // add the flag
      tmp += `<div class='flag-box'><div class='iti-flag ${
        c.iso2
      }'></div></div>`
    ...
    for (let i = 0; i < this.countries.length; i++) {
      if (this._startsWith(this.countries[i].name, query)) {
        const listItem = this.countryList.querySelector(
          `#iti-item-${this.countries[i].iso2}`
        )

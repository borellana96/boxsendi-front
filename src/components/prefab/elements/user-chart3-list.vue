      ...
      <data-table :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy" v-if="is_loading == false">
        <tbody>
          <tr v-for="(user) in users" :key="user.id">
            ...
            <td ><span >{{user.attributes.username}}</span></td>
...
export default {
  ...
  methods: {
    getUsers(page) {
      ...
      let url = process.env.API_URL + '/api/admin/list'
      ...
      axios
        ...
        .then(response => {
          ...
          if (this.tableData.draw === data.meta.draw) {
            this.users = data.data.admins_list
  ...
  computed: {
    filterColumns() {
      let activeFilterColumns = ['username', 'fullname']
